...
export function dockerPage({ t, icon, state, pageTitle, escapeHtml, backendReady }) {
  ...
  let filtered = d.containers.filter((c) => {
  ...
  return `
    <section class="page docker-page">
      ${pageTitle(t("docker.title"), t("docker.desc"), "docker")}
      ...
      <div class="workflow-panel">
        ${!isRemote ? `
          <div class="docker-control-row">
            <div class="field flex-1">
              <label for="docker-custom-root">${t("docker.customRoot")}</label>
              <div class="picker-input-row">
                <input type="text" id="docker-custom-root" placeholder="/var/lib/docker" value="${escapeHtml(d.customRoot)}" />
                <button type="button" class="tool-action-button secondary-button" data-docker-action="pick-custom-root">
                  ${icon("folder")}
                </button>
              </div>
        ...
        ` : `
          <div class="docker-remote-grid">
            <div class="field">
              <label for="docker-remote-ip">${t("docker.remoteHost")}</label>
              <input type="text" id="docker-remote-ip" placeholder="192.168.1.50" value="${escapeHtml(d.remote.ip)}" />
            </div>
            <div class="field">
              <label for="docker-remote-port">${t("docker.remotePort")}</label>
              <input type="number" id="docker-remote-port" placeholder="4444" value="${d.remote.port}" />
            </div>
            <div class="field">
              <label for="docker-remote-token">${t("docker.remoteToken")}</label>
              <input type="password" id="docker-remote-token" placeholder="••••••••" value="${escapeHtml(d.remote.token)}" />
            </div>
            <div class="button-row align-end">
              <button class="primary-button" data-docker-action="scan-remote" ${d.isScanning ? "disabled" : ""}>
                ${icon("network")} ${d.isScanning ? t("docker.scanning") : t("docker.remoteConnect")}
              </button>
            </div>
          </div>
        `}
        ...
        ${d.status ? `
          <div class="docker-status-cards">
            <div class="status-card">
              <span class="status-card-label">Docker Daemon</span>
              <span class="status-card-value ${d.status.docker_running ? "status-online" : "status-offline"}">
                ${d.status.docker_running ? "🟢 " + t("docker.running") : (d.status.docker_available ? "🟡 Disk / Offline" : "⚪ " + t("docker.stopped"))}
              </span>
            </div>
            <div class="status-card">
              <span class="status-card-label">${t("docker.total")}</span>
              <span class="status-card-value">${d.status.containers_count || d.containers.length}</span>
            </div>
            <div class="status-card">
              <span class="status-card-label">${t("docker.running")}</span>
              <span class="status-card-value status-online">${d.status.running_count}</span>
            </div>
            <div class="status-card">
              <span class="status-card-label">${t("docker.stopped")}</span>
              <span class="status-card-value status-muted">${d.status.stopped_count}</span>
            </div>
            <div class="status-card">
              <span class="status-card-label">Driver</span>
              <span class="status-card-value">${escapeHtml(d.status.storage_driver || "overlay2")}</span>
            </div>
          </div>
        ` : ""}
      </div>
      ...
      <div class="workflow-panel">
        <div class="panel-header-row">
          <h3>${icon("docker")} ${t("docker.containersTitle")} (${filtered.length})</h3>
          <div class="docker-filter-group">
            <input type="text" class="docker-search-input" placeholder="${t("docker.searchPlaceholder")}" value="${escapeHtml(d.search)}" data-docker-action="search" />
            <div class="filter-pill-buttons">
              ...
              <button class="filter-pill filter-pill-danger ${d.filter === "high_risk" ? "active" : ""}" data-docker-action="set-filter" data-filter="high_risk">⚠️ ${t("docker.filterHighRisk")}</button>
            </div>
          </div>
        </div>
        ...
        ${filtered.length === 0 ? `
          <div class="empty-state-box">
            ...
            ${!d.status ? `<button class="primary-button" data-docker-action="${isRemote ? "scan-remote" : "scan-local"}">${icon("search")} ${t("docker.scan")}</button>` : ""}
          </div>
        ` : `
    ...
    <tr class="docker-row ${c.risk_level === "CRITICAL" || c.risk_level === "HIGH" ? "row-highlight-risk" : ""}">
      ...
      <td>
        <div class="container-name-cell">
          ...
          <code>${escapeHtml(c.short_id || c.id.substring(0, 12))}</code>
        </div>
      </td>
      <td>
      ...
      <td>
        <div class="row-action-buttons">
          ...
          <button class="small-button primary-button" data-docker-action="acquire" data-id="${c.id}" data-name="${escapeHtml(c.name)}">
      ...
      <div class="docker-modal-content" onclick="event.stopPropagation()">
        <div class="modal-header">
          <div class="modal-title-group">
            <h2>${icon("docker")} ${escapeHtml(c.name)} <code>(${c.short_id || c.id.substring(0, 12)})</code></h2>
        ...
        <div class="modal-footer">
          ...
          <button class="primary-button" data-docker-action="acquire" data-id="${c.id}" data-name="${escapeHtml(c.name)}">
  ...
  return `
    <div class="inspector-section">
      <div class="security-score-banner ${c.risk_level === "CRITICAL" ? "bg-critical" : c.risk_level === "HIGH" ? "bg-high" : "bg-low"}">
        <h3>🛡️ Güvenlik Değerlendirmesi: ${c.risk_level} RISK</h3>
        <p>Amele, konteyner kaçış (breakout) ve host ele geçirme risklerini denetledi.</p>
      </div>

      <h4 class="inspector-subtitle">⚠️ Tespit Edilen Risk Faktörleri (${c.risk_reasons?.length || 0})</h4>
      ...
      ` : `
        <ul class="risk-reasons-list">
          ${c.risk_reasons.map(r => `<li><span class="warning-icon">⚠️</span> ${escapeHtml(r)}</li>`).join("")}
        </ul>
      `}
    </div>
  `;
...
export async function handleDockerAction(e, { apiRequest, setRoute, render }) {
  ...
  if (action === "set-mode") {
    ...
    const found = dockerState.containers.find(c => c.id === cid);
    if (found) {
...
async function scanLocalDocker({ apiRequest, render }) {
  ...
  try {
    ...
    if (statusRes?.status) {
      dockerState.status = statusRes.status;
    }
    ...
    if (containersRes?.containers) {
      dockerState.containers = containersRes.containers;
